...
updateLogFile:
	('updateLogFile' 'as' name=STRING 'from' source=STRING  'logfile' logfile=STRING  'using'
	'{'
		value=STRING
	'}' ('on-condition' condition=expression)* ('fielding' writeThreadCount=NON_ZERO_DIGIT)* ('with-fetch-batch-size' fetchBatchSize=NON_ZERO_DIGIT)* ('with-write-batch-size' writeBatchSize=NON_ZERO_DIGIT)*)
	 ;
...
restApi:
	'restapi' 'as' name=STRING 'source' source=STRING 'using' 'url' url=STRING 'method' method=STRING
	 ('with headers'  headers=json)* ('with params'  param=json )*
...
consumer:
    'consumer''as' name=STRING  ( 'pop-event-from' events=STRING 'on-resource' source=resource )*  ('pop-result-from' pop=STRING 'limit' limit=STRING)* 'execute' '{'
       (actions+=action)*
...
getDocListfromXDB:
	'getDocListfromXDB' 'as' name=STRING 'from' source=STRING 'rootpath' rootpath=STRING
	'schema' schema=STRING 'payload' table=STRING
	'status' status=STRING 'process-id' processId=STRING 'postgres' postgres=STRING  'metabatch' batchSize=STRING
	'using' '{' value=STRING '}' ('on-condition' condition=expression)*;


writeXdbToLocal:
        'writeXdbToLocal' 'as' name=STRING 'from' source=STRING 'postgres' postgres=STRING 'path' path=STRING
        'rootpath' rootpath=STRING 'sourceschema' sourceschema=STRING 'sourcetable' sourcetable=STRING
        'destinationschema' destinationschema=STRING 'destinationtable' destinationtable=STRING 'sTimeout' sessionTimeout=STRING
        'fielding' batchSize=STRING 'on-parallel-fielding' threadCount=STRING
        '{' value=STRING '}' ('on-condition' condition=expression)*;
pushLocalToMl:
         'pushLocalToMl' 'as' name=STRING 'from' source=STRING 'to' mlConnection=STRING
         'destinationschema' destinationschema=STRING 'destinationtable' destinationtable=STRING
         'fielding' batchSize=STRING 'on-parallel-fielding' threadCount=STRING
         '{' value=STRING '}' ('on-condition' condition=expression)*;
